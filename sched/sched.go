...
func parseError(errorString string, input []byte, start int, end int) error {
...
func (p *ScheduleParser) expect(ty TokenType) (tok *token, err error) {
	...
	if tok == nil {
		tok := p.peek()
		err = parseError(fmt.Sprintf("expected token %v, got %v", ty, tok.ty), p.input, tok.start, tok.end)
	}
	return
}
...
func (p *ScheduleParser) parseWeekday() (weekday time.Weekday, err error) {
	if p.accept(MON) != nil {
		...
		tok := p.peek()
		err = parseError(fmt.Sprintf("Expected day of week, got %v", p.peek().ty), p.input, tok.start, tok.end)
	}
	return
}
